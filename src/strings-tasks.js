...
function removeFirstOccurrences(str, value) {
  const indexFirst = str.indexOf(value);
  if (indexFirst === -1) {
    return str;
  }
  return str.slice(0, indexFirst).concat(str.slice(indexFirst + value.length));
}
...
function removeLastOccurrences(str, value) {
  ...
  return str.slice(0, indexLast).concat(str.slice(indexLast + value.length));
}
...
function formatTime(minutes, seconds) {
  const minutStr = String(minutes).padStart(2, '0');
...
function countVowels(str) {
  ...
  return str.split('').filter((test) => vowels.includes(test)).length;
}
...
function isPalindrome(str) {
  ...
  reverseStr = reverseStr.replaceAll(/[!?, ]/g, '');
  if (reverseStr === str.replaceAll(/[!?, ]/g, '').toLowerCase()) {
    return true;
  }
  return false;
}
...
function findLongestWord(sentence) {
  return sentence.split(' ').reduceRight((longWord, currentword) => {
    if (longWord.length > currentword.length) {
      return longWord;
    }
    return currentword;
  }, '');
...
function reverseWords(str) {
  const reverseSt = str.split('').reverse().join('');
  return reverseSt.split(' ').reverse().join(' ');
}
...
function encodeToRot13(str) {
  ...
  const result = str.split('').map((item) => {
    const indexItem = firstStr.indexOf(item);
    if (item === ' ' || item === '?' || item === '!') {
      return item;
    }
    return secondStr[indexItem];
  });
  return result.join('');
}
